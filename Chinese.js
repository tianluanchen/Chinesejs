...
; !function (global, factory) {
     ...
     */
    function toSimple(str, counter, promise) {
        if (Boolean(promise)) {
     ...
     * @return {Object|String|Promise}
     */
    function toTraditional(str, counter, promise) {
        if (Boolean(promise)) {
            return new Promise(function (resolve, reject) {
    ...
    legalTypes.string = '[Node.DOCUMENT_NODE, Node.DOCUMENT_FRAGMENT_NODE, Node.TEXT_NODE, Node.ELEMENT_NODE]';

    /**
     * @description: 检查DOMnode类型合法否
     * @param {Node} node
     * @return {void}
     */
    function checkNodeType(node) {
        if (!legalTypes.includes(node.nodeType)) {
            throw new Error('illegal NodeType,legal NodeType:'.concat(legalTypes.string));
        }
    }
    ...
    */
     ...
     * @return {Object}
     */
    function transDOMToSimple(node) {
        return new Promise(function (resolve, reject) {
            var parent = node ? node : document;
            checkNodeType(parent);
            var nodeList = Array.prototype.slice.call(parent.childNodes);
     ...
     * @return {Object}
     */
    function transDOMToTraditional(DOMNode) {
        return new Promise(function (resolve, reject) {
    ...
    function showWidget(opts = {
        ...
        if (['zh-CN', 'zh-Hans', 'zh-SG', 'zh-MY'].includes(navigator.language)) {
            simple = true;
        }
        ...
        function touch(event) {
            if (event.target.className.indexOf('clang-menu-hide') > -1) {
                show(event);
            } else {
                click(event);
            }
    ...
    var Chinesejs = {
        toSimple, toTraditional, legalTypes, transDOMToSimple, transDOMToTraditional, autoTranslate, restore, showWidget
    };
    return Chinesejs;
});