...
function autoTranslate(dom = document, output = false) {
    ...
    return new Promise(function (resolve, reject) {
        let current = env_check_1.SIMPLE ? "简" : env_check_1.TRADITIONAL ? "繁" : "";
        ...
        if (current != "" && target != undefined) {
            (0, dom_translate_1.translateDOM)(dom, target).then((result) => {
                if (output) {
...
function enableLittleMenu({ leftTime = 3000, text = "简", translate = undefined, target = document, customStyle = "", customHideStyle = "", callback = undefined }) {
    ...
    function trans() {
        if (index === 2) {
            (0, dom_translate_1.restoreDOM)(target).then(count => {
                let result = {
                    current: "原",
                    charCount: undefined,
                    nodeCount: count,
            ...
            (0, dom_translate_1.translateDOM)(target, targetObj[text]).then((result) => {
                result.current = text;
                callback && callback(result);
            });
        }
